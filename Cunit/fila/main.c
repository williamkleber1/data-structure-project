...
void testefila(void) //função que executa os testes para pilha
{
    ...
    CU_ASSERT(1 == is_empty(fila));
    //CU_ASSERT(0 == is_empty(fila));

    //função dequeue retorna -1 caso se não tiver nada enfileirado
    CU_ASSERT(-1 == dequeue(fila));
   
 
    for (i = 1; i <= 100; i++)
        enqueue(fila,i);       //enfileira os numeros de 1 a 100
